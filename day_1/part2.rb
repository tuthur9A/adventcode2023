...
input.each do |i|
    arrayLine = i.split("")
    replaceLine = ""
    arrayLine.each_with_index do |char, charIndex|
      if char.match(/\d/) then
        replaceLine += char
      end
      numbersSpelledOut.each_with_index do |num, numIndex|
        if i[charIndex..(charIndex -1 + numbersSpelledOut[numIndex].length)] == num then
          replaceLine += numIndex.to_s
        end
      end
    end
    final = replaceLine[0] + replaceLine[replaceLine.length - 1]
    result += final.to_i
