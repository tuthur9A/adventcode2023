input = 'Game 1: 1 green, 1 blue, 1 red; 3 green, 1 blue, 1 red; 4 green, 3 blue, 1 red; 4 green, 2 blue, 1 red; 3 blue, 3 green
...
class Set
    def initialize(id, red, blue, green)
        @id = id
        @red = red
        @blue = blue
        @green = green
    ...
    def to_s
        "Set " + @id.to_s  + " " + @red.to_s + " red " + @blue.to_s + " blue " + @green.to_s + " green " 
     end
    def red()
        return @red
    end
...
class Game 
    ...
    def checkIfPossible(red, blue, green)
        reds = @sets.map { |set| set.red }
        testRed = reds.select {|num| red < num}
        blues = @sets.map { |set| set.blue }
        testBlue = blues.select {|num| blue < num}
        greens = @sets.map { |set| set.green }
        testGreen = greens.select {|num| green < num}
        if testRed.length == 0 && testBlue.length == 0 && testGreen.length == 0
            return true
        end
        return false
    end
...
def formatInput(input) 
    ...
    input.split("\n").each do |line|
        ...
        sets = []
        line.split(":")[1].split(";").each_with_index do |set, setIndex|
            red = 0
            blue = 0
            green = 0
            set.split(",").each do |cube|
                case cube.split(" ")[1]
                when "red"
                    red = cube.split(" ")[0].to_i
                when "blue"
                    blue = cube.split(" ")[0].to_i
                when "green"
                    green = cube.split(" ")[0].to_i
                end
            end
            sets.push( Set.new(setIndex + 1,red,blue,green))
        end
        games.push(Game.new(gameId, sets));
    end
    return games
end

def checkGameOk(games) 
    result = 0
    gamesPossible = []
    games.each do |game|
        if game.checkIfPossible(12, 14, 13)
            gamesPossible.push(game.id)
            result += game.id
        end
    end
    print result.to_s
